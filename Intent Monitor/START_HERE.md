...
2. In the monitor, click **Stop**, select **Webcam**, choose your camera by its actual device name from the dropdown, and click **Start**.
3. If opening the camera fails, enable camera access and desktop-app camera access in Windows Settings. Close other applications using the camera. After connecting an external camera, click **Refresh** and select its name.
...
The camera dropdown shows detected device names, including virtual cameras. Click Refresh after connecting or disconnecting a camera. Identically named devices receive a suffix to distinguish them. If no cameras are detected, webcam Start is disabled; video mode remains available. Keep the bundled _vendor folder and camera_devices.py alongside the application. The selected device is opened explicitly without falling back to a different camera.
