...
class CNNLSTM(nn.Module):
    ...
    def __init__(self, num_classes: int = 3) -> None:
        super().__init__()
        # The supplied checkpoint contains the complete ResNet-101 state.
        # Starting without ImageNet weights avoids an unnecessary network
        # download before those parameters are immediately replaced.
...
class _OpenCVCamera:
    """Windows-compatible OpenCV camera with backend fallbacks."""
    ...
    def __init__(self, index: int = 0,
        ...
        if self._cap is None:
            raise RuntimeError(
                f"Windows camera index {index} failed with: {', '.join(errors)}"
            )

        self._index = index
        log.info("Camera: OpenCV %s index %d", self._backend, index)
    ...
    @property
    def backend_name(self) -> str:
        return f"opencv_{self._backend.lower().replace(' ', '_')}_{self._index}"


class _VideoFileCamera:
    """Play a video through the same sliding-window path as a live camera."""
    ...
    def __init__(self, path: str, loop: bool = True) -> None:
        ...
        self._frame_interval = 1.0 / source_fps if source_fps > 0 else 1.0 / 30.0
        self._last_read = 0.0
        self._path = path
        self._loop = loop
        log.info("Camera: video file %s (%.2f fps, loop=%s)",
                 path, source_fps, loop)
    ...
    def read(self) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        ret, frame = self._cap.read()
        if (not ret or frame is None) and self._loop:
    ...
    @property
    def backend_name(self) -> str:
        return f"video_{os.path.basename(self._path)}"
...
class CameraFactory:
    ...
    def open(width: int = 640, height: int = 480, fps: int = 30,
             camera_index: int = 0, video_path: str = "",
             loop_video: bool = True):
        errors = []

        if video_path:
            return _VideoFileCamera(video_path, loop=loop_video)

        try:
            return _RSSdkCamera(width, height, fps)
        except Exception as e:
            errors.append(f"RealSense: {e}")
            next_backend = "Windows OpenCV camera" if os.name == "nt" else "V4L2 camera"
            log.warning("RealSense unavailable, trying %s ...", next_backend)

        if os.name == "nt":
            try:
                return _OpenCVCamera(camera_index, width, height, fps)
            except Exception as e:
                errors.append(f"OpenCV@{camera_index}: {e}")
        else:
            v4l2_indices = list(dict.fromkeys([camera_index, 4, 0]))
            for index in v4l2_indices:
                try:
                    return _V4L2Camera(index, width, height, fps)
                except Exception as e:
                    errors.append(f"V4L2@{index}: {e}")
...
class IntentEngine:
    ...
    def __init__(
        ...
        if len(self.class_labels) != cfg.n_classes:
            raise ValueError(
                f"Expected {cfg.n_classes} class labels, got {len(self.class_labels)}: "
                f"{self.class_labels}"
            )
    ...
    def start(self, camera=None) -> None:
        if self._running:
            return
        log.info("Opening camera ...")
        self._camera        = camera if camera is not None else CameraFactory.open(
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--video_path",            default="",
                   help="Optional video file to use instead of a live camera")
    p.add_argument("--no_loop_video",         action="store_true",
                   help="Do not restart video input after its last frame")
    p.add_argument("--run_seconds",           type=float, default=0.0,
                   help="Stop automatically after this many seconds; 0 runs until Ctrl-C")
    p.add_argument("--status_interval",       type=float, default=0.5,
                   help="Seconds between console status refreshes")
...
def main() -> None:
    ...
    try:
        ...
        while True:
            s = engine.get_state()
            now = time.monotonic()
            if now - last_status_at >= max(args.status_interval, 0.05):
                bars = "  ".join(
                    f"{lbl}:{p*100:4.0f}%"
            ...
            if args.run_seconds > 0 and now - started_at >= args.run_seconds:
                print("\n[INFO] Requested runtime completed.")
                break
